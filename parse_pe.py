...
def read_eat(f, optional_header, section_headers):
    export_directory_table_offset = rva_to_offset(
        optional_header.export_directory[0], section_headers)

    if export_directory_table_offset > -1:
        f.seek(export_directory_table_offset)
        img_export_dir_tbl = ExportDirectory(f.read(40))

        name_offset = rva_to_offset(
            img_export_dir_tbl.name_rva, section_headers)
        ...
        if name_offset > -1:
            ...
            name_byte = int.from_bytes(f.read(1))
            while name_byte != 0:
                name_bytes.append(name_byte)
                name_byte = int.from_bytes(f.read(1))

            name = bytes(name_bytes).decode()

        eat_function_offsets = []
        eat_ordinals = []
        eat_function_name_rvas = []
        eat_function_rvas = []
        eat_function_names = []
        eat_function_forwarders = []

        eat_function_offset = rva_to_offset(
            img_export_dir_tbl.address_of_functions, section_headers)

        if eat_function_offset > -1:
            for _ in range(img_export_dir_tbl.number_of_functions):
                f.seek(eat_function_offset)

                eat_function_offsets.append(eat_function_offset)

                eat_function_rva = int.from_bytes(f.read(4), 'little')
                eat_function_rvas.append(eat_function_rva)

                eat_function_offset += 4

        eat_ordinal_offset = rva_to_offset(
            img_export_dir_tbl.address_of_name_ordinals, section_headers)

        if eat_ordinal_offset > -1:
            for _ in range(img_export_dir_tbl.number_of_names):
                f.seek(eat_ordinal_offset)

                eat_ordinals.append(int.from_bytes(f.read(2), 'little'))

                eat_ordinal_offset += 2

        eat_function_name_rva_offset = rva_to_offset(
            img_export_dir_tbl.address_of_names, section_headers)

        if eat_function_name_rva_offset > -1:
            for _ in range(img_export_dir_tbl.number_of_names):
                f.seek(eat_function_name_rva_offset)

                function_name_rva = int.from_bytes(f.read(4), 'little')
                eat_function_name_rvas.append(function_name_rva)

                function_name_offset = rva_to_offset(
                    function_name_rva, section_headers)

                f.seek(function_name_offset)
                name_chr = int.from_bytes(f.read(1))
                name_bytes = []

                while name_chr != 0:
                    name_bytes.append(name_chr)
                    name_chr = int.from_bytes(f.read(1))

                eat_function_names.append(bytes(name_bytes).decode('utf8'))

                eat_function_name_rva_offset += 4

        return img_export_dir_tbl, eat_ordinals, eat_function_rvas, eat_function_names


def read_iat(f, optional_header, section_headers):
    iat_directory_table_offset = rva_to_offset(
        optional_header.import_directory[0], section_headers)

    iat_directory_tables = []

    f.seek(iat_directory_table_offset)
    import_directory_table = ImportDirectoryTable(f.read(20))

    while not import_directory_table.is_zero():
        iat_directory_tables.append(import_directory_table)
        import_directory_table = ImportDirectoryTable(f.read(20))

    for iat_dir_table in iat_directory_tables:
        name_offset = rva_to_offset(
            iat_dir_table.name_rva, section_headers)
        ...
        import_lookup_table_offset = rva_to_offset(
            iat_dir_table.import_lookup_table_rva, section_headers)

        print(bytes(name_bytes).decode('utf-8'))
