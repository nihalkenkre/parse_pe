...
class ImportDirectory:
    def __init__(self, import_directory_bytes: bytes = None, f=None, section_headers: list[SectionHeader] = None, pe_type: PEType = None, print_function_count: int = 10) -> None:
        ...
        self.import_lookup_table = []  # Original Thunk
        self.hint_name_table = []
        self.ordinals = []
        self.import_address_table = []  # First Thunk
        ...
        for idx in range(print_function_count):
            ...
            if import_lookup_table_entry != 0:
                ...
                import_address_table_rva = self.import_address_table_rva + \
                    (idx * import_lookup_table_entry_size)
                self.import_address_table.append(import_address_table_rva)
        ...
        return_str = f'\n\
            Import Directory - {self.name}\n\
                ...
                Import Addr Table RVA       : {hex(self.import_address_table_rva)}\n\
                        \n\
                        Imported Functions\n\
Ordinal     Hint      Name                                Original Thunk     Call Via\
    '
        for idx in range(len(self.hint_name_table)):
            return_str += f'\n\
{str(hex(self.ordinals[idx]).ljust(7))}     {str(hex(self.hint_name_table[idx].hint)).ljust(1)}      {self.hint_name_table[idx].name.ljust(30)}         {hex(self.import_lookup_table[idx])}            {hex(self.import_address_table[idx])}\
        '
